 ...
 * See https://projecteuler.net/problem=587
 * Copyright (c) Project Nayuki. All rights reserved.
 *
 * https://www.nayuki.io/page/project-euler-solutions
 * https://github.com/nayuki/Project-Euler-solutions
 *
 * Note: I don't understand the algorithm, I am just converting the code
 */
...
const IMAX:u32 = 2147483647;
	 
fn run() -> u32 {
	let left_section_area:f64 = 1.0 - PI / 4.0;
	let mut i:u32 = 1;
	//convert the for loop to an indefinite loop
	loop {
		let slope:f64 = 1.0 / i as f64;
		let a = slope * slope + 1.0;
		let b = -2.0 * (slope + 1.0);
		let c:f64 = 1.0;
		let xq = (b * b - 4.0 * a * c).sqrt();
		let x = (2.0 * c) / (-b + xq);
		let concave_triangle_area = (x * x * slope / 2.0) + (integral(1.0) - integral(x));
		if concave_triangle_area / left_section_area < 0.001 {
			return i;
		}
		i = i+1;
		if i == IMAX {
			println!("unable to solve; overflow");
			return i;
		}
	}
}
